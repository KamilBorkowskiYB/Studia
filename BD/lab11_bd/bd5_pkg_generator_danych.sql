/*****************************************************
-- Programowy generator danych dla modelu BD5_RACHUNEK
--
-- opr. Józef WoŸniak (2020)
******************************************************/

create or replace package bd5_pkg_generator_danych is

-- procedura generuj¹ca jedn¹ transakcjê poprzez generator
procedure pr_generuj_transakcje (v_data_sprzedazy date default sysdate);

-- procedura generuj¹ca wiele transakcji z jedn¹ dat¹ poprzez generator
procedure pr_generuj_transakcje (v_data_sprzedazy date default sysdate,
                                 v_ile number);

-- procedura generuj¹ca wiele transakcji w zadanym przedziale czasu poprzez generator 
...
-- funkcja losuje liczbê transakcji wygenerowanych jednej doby
function fn_daj_liczbe_transakcji(min_liczba number default 1,
                                  max_liczba number default 5) return number;                               

end bd5_pkg_generator_danych;
/
create or replace package body bd5_pkg_generator_danych as
  ...
  begin
	insert into bd5_rachunek values (v_nr_rachunku, v_data_sprzedazy,
  ...
  begin
    ...
    from (
            select nr_klienta from  bd5_klient
  ...
  begin
    ...
    from (
            select nr_produktu from  bd5_produkt
            order by dbms_random.value)
    where rownum = 1;
