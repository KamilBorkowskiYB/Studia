...
void kompresja()
{
    unsigned int bajtyWPlikuWejscia=dokumentacja.bajtyNum, bajtyWSum=0,bajtyKodu=0, bajtyKoduSum=0;
    unsigned char buforWejscia[BUFSIZE],buforWyjscia[BUFSIZE],symbol, pom, bajtOut=0;

    int kursorSlowka, kursorOut=0, brancheSuma = dokumentacja.dodaneSymbole;
    int liczbaBitowSlowaKodu, bityWolne, liczbaPozostaleBitySlowka, numerSlowka=0;

    int n,i,m,k;
    unsigned char maska1, maska2, maska, word[4],four[4];

	FILE *wskaznikPlikuOut=fopen(plikWyjsciowy,"wb+");
	...
	fwrite(&bajtyKodu,sizeof(unsigned int),1,wskaznikPlikuOut);
	fwrite(&bajtyKoduSum,sizeof(unsigned int),1,wskaznikPlikuOut);
	fwrite(&bajtyWPlikuWejscia,sizeof(int),1,wskaznikPlikuOut);
	printf("Liczbe bajtow przed kompresja wpisana do .Huffman %d\n", bajtyWPlikuWejscia);
	fwrite(&brancheSuma,sizeof(int),1,wskaznikPlikuOut);
	printf("Liczba galezi drzewa kodowania wpisana do .Huffman %d\n", brancheSuma);
    fwrite(drzewo,sizeof(struct wezel), brancheSuma, wskaznikPlikuOut);
    printf("Drzewo kodowania binarnie wpisana do .Huffman\n");
    printf("Symbole zakodowano i zapisano binarny strumien kodu do .Huffman\n");
    FILE *wskaznikPlikuIn=fopen(plikWejsciowy,"rb");
    if (wskaznikPlikuIn == NULL)
	{
		printf("Nie mozna otworzyc pliku: %s  \n", plikWejsciowy);
    ...
    while(n=fread(buforWejscia,sizeof(unsigned char),BUFSIZE,wskaznikPlikuIn))
    {
        for(i=0; i<n; ++i)
        {
            ...
            for (m=0; m<dokumentacja.symboleNum; ++m)
            {
                if (symbol==tablicaKodu[m].symbol)
                {
                for (k=0; k<4; ++k)
                    word[k]=tablicaKodu[m].slowo[k];
                liczbaBitowSlowaKodu=tablicaKodu[m].liczbaBitow;
                bajtyKoduSum=bajtyKoduSum+liczbaBitowSlowaKodu;
                kursorSlowka=0;
                numerSlowka=0;
                break;
                }
            ...
            while (liczbaBitowSlowaKodu>0)
            {
                ...
                bityWolne=8-kursorOut;
                if(liczbaPozostaleBitySlowka<bityWolne){
                    bityWolne=liczbaPozostaleBitySlowka;
                }
                maska1=255>>kursorSlowka;
                maska2=255<<(8-(kursorSlowka+bityWolne));
                maska=maska1&maska2;
                pom=word[numerSlowka]&maska;
                pom=pom<<kursorSlowka;
                pom=pom>>kursorOut;
                bajtOut=bajtOut|pom;
                kursorOut+=bityWolne;
                liczbaBitowSlowaKodu-=bityWolne;
                kursorSlowka+=bityWolne;
                liczbaPozostaleBitySlowka-=bityWolne;
    ...
    printf("W pliku wejsciowym przerobiono: %d bajtow\n", bajtyWSum);
    printf("W pliku skompresowanym jest: %d bajtow\n", bajtyKodu);

    printf("W pliku skompresowanym jest: %d bitow\n", bajtyKoduSum);
    printf("Wskaznik upakowania: %5.1f  procent\n", 100*(float)(bajtyKodu)/(float)bajtyWSum);
